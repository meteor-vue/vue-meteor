...
export default function patchSubscribeData (VueSSR) {
  ...
  Mongo.Collection.prototype.find = function (selector = {}, options = {}) {
    selector = selector || {}
    const ssrContext = VueSSR.ssrContext.get()
    if (ssrContext && !VueSSR.inSubscription.get()) {
  ...
  ['call', 'apply'].forEach((methodName) => {
    ...
    Meteor[methodName] = (...args) => {
      const response = VueSSR.ssrContext.withValue(null, () => {
